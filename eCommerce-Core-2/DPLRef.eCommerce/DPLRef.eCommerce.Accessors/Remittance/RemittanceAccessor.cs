...
namespace DPLRef.eCommerce.Accessors.Remittance
{
    class RemittanceAccessor : AccessorBase, IRemittanceAccessor
    {
        ...
        public decimal SalesTax(string zipCode, DateTime start, DateTime end)   // noel  add sales tax report by zip
        {
            using var db = eCommerce.Accessors.EntityFramework.eCommerceDbContext.Create();

            /*var SalesTax = (from o in db.Orders
                            where o.BillingPostal == zipCode &&
                            (o.CreatedAt >= start && o.CreatedAt <= end)
                            orderby o.BillingPostal
                            select o.TaxAmount).Sum();*/
            
            return (from o in db.Orders                                         // noel
                    where o.BillingPostal == zipCode &&
                    (o.CreatedAt >= start && o.CreatedAt <= end)
                    orderby o.BillingPostal
                    select o.TaxAmount).Sum(); ;
